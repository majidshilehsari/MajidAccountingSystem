// اطلاعات اتصال به Supabase (هماهنگ با نسخه docs)
...
// تنظیم پیش‌فرض تاریخ شمسی امروز روی input و اعتبارسنجی
...
async function fetchPersons() {
    const { data, error } = await supabase.from('persons').select('*').order('id', { ascending: true });
    if (error) {
...
async function fetchRecords() {
    ...
    const { data, error } = await supabase.from('records').select('*').eq('person_id', personId).order('id', { ascending: true });
    if (error) {
...
async function editRecordPrompt(record) {
    ...
    await supabase.from('records').update({ desc: newDesc, amount: parseInt(newAmount, 10) }).eq('id', record.id);
    fetchRecords();
}
